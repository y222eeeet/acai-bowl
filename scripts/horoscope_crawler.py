...
1) 전체를 자연스러운 한국어로 번역해 주세요. 일본어 한자(漢字)는 반드시 한글로 옮기세요 (예: 突破口→돌파구). 문체는 해요체(존댓말)로 통일하고, 문장은 마침표 뒤에 공백을 넣어 구분해 주세요. 모든 문장 끝에 반드시 마침표(.)를 붙여 주세요. 문장 끝은 항상 ~요/~예요/~이에요처럼 존댓말로 쓰고, ~야, ~이야 등 반말은 절대 사용하지 마세요.
...
# 배치용 짧은 지시문
_BATCH_PROMPT_PREFIX = """12개 일본어 별자리 운세를 한국어로 번역. 일본어 한자(漢字)는 반드시 한글로 옮기기 (예: 突破口→돌파구). 해요체(존댓말). 모든 문장 끝에 반드시 마침표(.) 붙이기. ~야/~이야 등 반말은 절대 사용하지 않기. 문장 구분은 마침표 뒤 공백.
