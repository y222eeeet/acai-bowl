...
1) 전체를 자연스러운 한국어로 번역해 주세요. 일본어 한자(漢字)는 반드시 한글로 옮기세요 (예: 突破口→돌파구).
2) 문체는 반드시 해요체(~해요, ~예요, ~이에요)로만 작성. 합니다/됩니다/입니다 등 하십시오체는 절대 사용하지 마세요. 문장 구분은 마침표+공백. 모든 문장 끝에 마침표(.).
3) 상세(detail) 내용을 반영한 한줄평 한 문장을 만들어 주세요. 20자 내외, 자연스럽게 ~해요/~예요/~이에요로 끝내세요.
...
# 배치용 짧은 지시문
_BATCH_PROMPT_PREFIX = """12개 일본어 별자리 운세를 한국어로 번역. 일본어 한자(漢字)는 한글로 옮기기 (예: 突破口→돌파구).
문체는 반드시 해요체(~해요, ~예요, ~이에요)만 사용. 합니다/됩니다/입니다 등 하십시오체 사용 금지. 문장 구분은 마침표+공백. 모든 문장 끝에 마침표.
각 항목 한줄평은 20자 내외, 자연스럽게 ~해요/~예요/~이에요로 끝내기.
